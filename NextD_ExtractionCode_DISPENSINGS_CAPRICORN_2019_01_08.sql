...
select i.PATID,
		...
		i.PRESCRIBINGID,
		i.NDC,
		i.DISPENSE_DATE_YEAR,
		i.DISPENSE_DATE_MONTH,
		i.DISPENSE_DATE_DAYSFROMFIRSTVISIT,
		i.DISPENSE_SUP,
		i.DISPENSE_AMT,
		i.RAW_NDC 
into #NextD_DISPENSING
from (select c.PATID,
b.DISPENSINGID,
b.PRESCRIBINGID,
b.NDC,
...
year(b.DISPENSE_DATE) as DISPENSE_DATE_YEAR,
month(b.DISPENSE_DATE) as DISPENSE_DATE_MONTH,
datediff(dd,c.FirstVisit,b.DISPENSE_DATE) as DISPENSE_DATE_DAYSFROMFIRSTVISIT,
b.DISPENSE_SUP,
b.DISPENSE_AMT,
b.RAW_NDC
from /* provide name of table 1 here: */ #Final_Table1 c 
left join  [capricorn].[dbo].[DISPENSING] b on c.PATID=b.PATID 
left join dbo.DEMOGRAPHIC d on c.PATID=d.PATID
where (datediff(yy,d.BIRTH_DATE,b.DISPENSE_DATE) between @LowerAge and @UpperAge) and (b.DISPENSE_DATE between @LowerTimeFrame and @UpperTimeFrame)
) i
...
from #NextD_DISPENSING i join  /* provide name the table containing c.PATID,Hashes, and Global patient id*/ #GlobalIDtable d on i.PATID=d.CAP_ID;
---------------------------------------------------------------------------------------------------------------
