---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
-----                             Part 12: PROVIDERS for the study sample                                -----  
--------------------------------------------------------------------------------------------------------------- 
---------------------------------------------------------------------------------------------------------------
/* Tables for this eaxtraction: 
1. Table 1 (named here #Final_Table1) with Next-D study sample IDs. See separate SQL code for producing this table.
2. PROVIDER table from PCORNET.
3.External source table (#NEXTD_NPI_Remap) with mappings between NPI values and taxonomy codes.
...
select b.PROVIDERID,
		b.PROVIDER_NPI,--optional if site is not populating those in PCORI table
		b.PROVIDER_SPECIALTY_PRIMARY,
		case when d.[Healthcare Provider Taxonomy Code_1] is not NULL and d.[Healthcare Provider Taxonomy Code_1] in ('102X00000X','171R00000X','172A00000X','247ZC0005X','367A00000X','374K00000X','374T00000X','376G00000X','390200000X') then '0'
			when d.[Healthcare Provider Taxonomy Code_1] is not NULL and d.[Healthcare Provider Taxonomy Code_1] in ('202C00000X','202K00000X','204C00000X','204D00000X','204E00000X','204F00000X','204R00000X','207K00000X','207KA0200X','207KI0005X','207L00000X','207LA0401X','207LC0200X',
						...
						'208VP0014X','209800000X') then 'Physician'
			when d.[Healthcare Provider Taxonomy Code_1] is not NULL and d.[Healthcare Provider Taxonomy Code_1] in ('363L00000X','363LA2100X','363LA2200X','363LC0200X','363LC1500X','363LF0000X','363LG0600X','363LN0000X','363LN0005X','363LP0200X','363LP0222X',
						...
						'364SP0810X','364SP0811X','364SP0812X','364SP0813X','364SP1700X','364SP2800X','364SR0400X','364SS0200X','364ST0500X','364SW0102X','364SX0106X','364SX0200X','364SX0204X','367500000X',
						'363A00000X','363AM0700X','363AS0400X') then 'APP'
			when d.[Healthcare Provider Taxonomy Code_1] is not NULL and d.[Healthcare Provider Taxonomy Code_1] in ('101Y00000X','101YA0400X','101YM0800X','101YP1600X','101YP2500X','101YS0200X','102L00000X','103G00000X','103GC0700X','103K00000X','103T00000X','103TA0400X','103TA0700X',
						...
						'2472V0600X','367H00000X','372500000X','372600000X','373H00000X','374700000X','3747A0650X','3747P1801X','374J00000X','374U00000X','376J00000X','376K00000X','405300000X','163W00000X',
						'163WA0400X','163WA2000X','163WC0200X','163WC0400X','163WC1400X','163WC1500X','163WC1600X','163WC2100X','163WC3500X','163WD0400X','163WD1100X','163WE0003X',
...
into #NextD_PROVIDER
from [dbo].[PROVIDER] b 
left join /* provide name of external source table with raw encounter types and NPI codes here: */ #NEXTD_NPI_Remap d on b.PROVIDERID=d.PROVIDERID --could be the same [dbo].[PROVIDER] table if NPI values are populated there 
;
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
select PROVIDERID,
		PROVIDER_NPI,--optional if site is not populating those in PCORI table
		PROVIDER_SPECIALTY_PRIMARY,
		IndividualProviderCategories
into #NextD_PROVIDER_FINAL
from #NextD_PROVIDER 
---------------------------------------------------------------------------------------------------------------
/* Save #NextD_PROVIDER_FINAL as csv file. 
